...
function TeamList(props) {
  return (
    <div className="teamList-card flex flex-col gap-2 p-4 mt-3 mb-3 w-2/5 rounded-lg shadow-md bg-gray-100">
      ...
      <button onClick={()=>{props.dispatch("Calculate_average_age")}}>Calculate Average Age</button>
        <p>Average age:</p>
      ...
      {props.data.map((employee) => (
        <div className="employee-card flex items-center p-3 rounded-md bg-white border border-gray-200 shadow-sm hover:shadow-lg" key={employee.id}>
          ...
          <button onClick={() => props.dispatch("Remove_from_team_list")} className="text-blue-500 font-bold border border-blue-500 px-2 py-1 rounded-md ml-auto">+</button>
        </div>
      ))}
    </div>
  )
